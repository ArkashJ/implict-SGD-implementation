...
euclidean_distance <- function(theta, theta_star) {
  sqrt(sum((theta - theta_star)^2))
}


plot_distances <- function(distances) {
  df <- data.frame(iteration=1:length(distances), distance=distances)
  ggplot(df, aes(x=iteration, y=distance)) +
    geom_line() +
    theme_minimal() + 
     theme(panel.background = element_rect(fill = "white", colour = "white"),
    axis.text = element_text(colour = "blue", size = 12, face = "bold") 
     ) +
     xlab("Iteration") +
     ylab("Euclidean Distance") +
     ggtitle("Euclidean Distance Across Iterations")
}
...
run <- function(model, pars, n=1e4, p=1e1, add.methods=NULL, plot.save=F, ...) {
  ...
  for (i in 1:pars.len) {
    ...
    sgd_results <- theta[[i]]
    theta_star <- sgd_results[, ncol(sgd_results)]
    distances <- apply(sgd_results, 2, function(theta) euclidean_distance(theta, theta_star))
    all_distances[[i]] <- distances
    plot_name <- sprintf("AI-SGD (%s, %s)", par[1], par[2])
  ...
  for (i in add.methods) {
    if (i %in% c("ADAM", "AMSGrad", "RMSprop")) {
      theta[[i]] <- sgd(d, sgd.method=i, lr=lr.explicit, p=p, ...) # Adjust as necessary
        sgd_results <- theta[[i]]
    theta_star <- sgd_results[, ncol(sgd_results)]
    distances <- apply(sgd_results, 2, function(theta) euclidean_distance(theta, theta_star))
    plot_names[[i]] <- i
    ...
    else if (i %in% c("SGD", "ASGD", "LS-SGD")) {
    ...
    plt <- plot_distances(distances)
    file_name <- sprintf("img/theta_plot_%s_n%ip%i.png", plot_names[[i]], log(n, base=10), log(p, base=10))
    ggsave(file_name, plot=plt)

  }
  
  for (i in seq_along(all_distances)) {
      plt <- plot_distances(all_distances[[i]])
      file_name <- sprintf("img/distance_plot_%s_n%ip%i.png", plot_names[[i]], log(n, base=10), log(p, base=10))
      ggsave(file_name, plot=plt)
    }
